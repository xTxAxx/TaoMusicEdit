...
│   ├── ffmpeg.py          # FFmpeg 封装：可执行文件定位、编码器探测、命令执行与进度解析
...
| `VALIDATION_ERROR` | 参数校验失败（如未提供或同时提供裁剪参数、超出时长/帧数等） |
| `OUTPUT_EXISTS` | 输出文件已存在且未确认覆盖 |
| `OVERWRITE_INPUT` | 输出路径与输入文件相同，禁止覆盖原文件 |
| `FFMPEG_EXECUTION_ERROR` | ffmpeg 执行失败 |
