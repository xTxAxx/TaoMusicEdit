...
- **区间裁剪**：终点为可选参数——留空（无）= 保留到片尾（原有行为）；设置终点后保留
  ...
  检测命中只自动填入起点、不触碰终点。
- **检测 / 裁剪异步任务**：后台线程执行，SSE 实时推送进度，播放器下方任务卡片展示
  每个文件的状态与进度，支持取消（整批 / 单个文件）。
...
> 命名且不提交）。想直接跑示例时无需改名：两个 `examples/usage_example.py` 会按
> 「`input*.mp4` 优先 → 仓库根目录任意 `.mp4`」的顺序自动定位可用视频，
> 且不依赖当前工作目录。
...
共 **209 个测试**（detector 84 / trimmer 106 / webui 19），实测 **193 通过、16 跳过**；
