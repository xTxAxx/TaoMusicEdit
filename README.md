...
- **批处理**：批量检测 / 批量裁剪分开执行；文件树支持「全选 / 清空」、
  Ctrl 单选、Shift 区间选择；批处理**文件级并行**（并发数可配置，见侧边栏「批处理」卡片）、
...
| `TAOMUSIC_BATCH_WORKERS` | `0` | 批处理并发数兜底值。优先取界面「批处理 → 并发数」（落盘时钳制到 0~16），其次取本变量（按原值使用），都为 0 时取 `min(CPU 核数, 4)` |
...
1. **载入工作区**：在左侧边栏「路径选择」卡片的「工作目录」行输入或浏览目录 → 点「载入」；
   右侧「文件选择」面板的文件树中展开目录并点击选择视频。
2. **设置输出目录**（可选）：在「路径选择」卡片的「输出目录」行指定裁剪结果保存位置，
   默认保存到项目根目录。
...
2. **批量检测**：点击「批量检测」按钮，对选中文件并行执行颜色检测（并发数见侧边栏
   「批处理」卡片），任务卡片实时显示每个文件与整体进度；结束后列出每个文件的检测结果
   （命中帧号 / 时间戳 / 未命中 / 失败原因），结果会自动缓存供批量裁剪使用；
   勾选侧边栏「批处理」卡片中的「跳过已缓存的检测」时，已有有效缓存结果的视频
   直接复用缓存、跳过重新检测。
...
- 保存内容：
  ...
  - 检测结果缓存配置（缓存开关、条目上限、跳过已缓存的检测）与批处理并发数。
...
- 管理入口：侧边栏「检测结果缓存」卡片可开关缓存、设置条目上限（0 = 不限，超出自动淘汰最旧的）、
  刷新统计、清理失效条目或一键清空。
