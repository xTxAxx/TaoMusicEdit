...
5. **Trim**：确认起点（可手动修改，或用「设为起点」捕获当前帧）；（可选）设置终点——
   直接输入终点值，或播放定位后点「设为终点」捕获当前帧（终点留空 = 保留到片尾，
   点击终点徽标或把终点方式切回「无」可清除终点）。点击 `Trim` 按钮完成裁剪，
   结果面板列出输出文件列表。
...
2. **批量 Detect**：点击「批量 Detect」按钮，对选中文件逐个执行颜色检测，
   ...
   勾选「跳过已缓存的检测」时，已有有效缓存结果的视频直接复用缓存、跳过重新检测。
3. **批量 Trim**：批量裁剪始终以每个文件的检测结果作为裁剪起点——
   优先使用本次「批量 Detect」的结果，其次匹配持久化缓存中的有效结果；
   无有效检测结果的文件会自动标记跳过、不参与裁剪。点击「批量 Trim」按钮
...
| `/api/detect-cache/config` | GET / POST | 读取 / 更新缓存配置（开关、条目上限） |
...
- 管理入口：侧边栏「检测结果缓存」面板可开关缓存、设置条目上限（0 = 不限，超出自动淘汰最旧的）、
  清理失效条目或一键清空。
...
detector 共 86 个测试、trimmer 共 106 个测试（含区间裁剪单元 / 集成用例）、
webui 共 19 个测试（`/api/trim` 终点参数校验等），全部通过；
